...
abstract class EnumMapBindAdapter<E : Enum<E>> : DataBindAdapter() {
    ...
    fun getDataBinder(viewType: Int): DataBinder<BaseViewHolder> {
        return getDataBinder(getEnumFromOrdinal(viewType))
    }

    fun getPosition(binder: DataBinder<*>, binderPosition: Int): Int {
    ...
    fun getBinderPosition(position: Int): Int {
    ...
    fun notifyBinderItemRangeChanged(binder: DataBinder<*>, positionStart: Int, itemCount: Int) {
        for (i in positionStart..itemCount) {
            notifyItemChanged(getPosition(binder, i))
        }
    }

    fun notifyBinderItemRangeInserted(binder: DataBinder<*>, positionStart: Int, itemCount: Int) {
        for (i in positionStart..itemCount) {
            notifyItemInserted(getPosition(binder, i))
        }
    }

    fun notifyBinderItemRangeRemoved(binder: DataBinder<*>, positionStart: Int, itemCount: Int) {
        for (i in positionStart..itemCount) {
